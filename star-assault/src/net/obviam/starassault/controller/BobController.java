...
public class BobController {
	...
	public void jumpReleased() {
		...
		jumpReleasedTime = System.currentTimeMillis();
	...
	public void update(float delta) {
		...
		if (bob.getPosition().y < 0) {
			bob.setPosition(new Vector2(0, 2));
		}
		...
		if (keys.get(Keys.JUMP)) {
			...
			if (jumpPressedTime != 0) {
				if (!bob.getState().equals(State.JUMPING)) {
					...
					long jumpTime = jumpReleasedTime - jumpPressedTime;
					if (jumpTime >= LONG_JUMP_PRESS) {
						jumpTime = LONG_JUMP_PRESS;
					}
					System.out.println(bob.getVelocity().x);
					System.out.println(bob.getVelocity().y);
					bob.getVelocity().y = MAX_JUMP_SPEED * (jumpTime / 100);
					bob.getVelocity().x = MAX_JUMP_SPEED * (jumpTime / 100);
					grounded = false;
				}
				jumpPressedTime = 0;
			}
